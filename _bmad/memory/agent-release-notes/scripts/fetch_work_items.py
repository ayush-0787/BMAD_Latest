...
DEFAULT_FIELDS = [
    ...
    # TWEHR/Allscripts custom fields: TargetRelease is the authoritative release
    # label for a work item and should be preferred over an owner-stated release
    # name if the two ever disagree. GoLiveBlocking/Regulatory are candidate
    # Breaking Change signals even though there's no dedicated tag for it.
    "Allscripts.Field.TargetRelease",
    "Allscripts.Field.InjectedinRelease",
    "Allscripts.Field.ReportedinRelease",
    "Allscripts.Field.GoLiveBlocking",
    "Allscripts.Field.Regulatory",
]
BATCH_SIZE = 200
...
def read_pat(pat_env: str | None) -> str:
    if pat_env:
        ...
        return pat.strip()
    # Read raw bytes rather than through sys.stdin's text decoder: on Windows,
    # PowerShell prefixes piped stdin with a UTF-8 BOM, and the console's text
    # codec can decode it as three stray Latin-1 characters instead of one
    # U+FEFF. utf-8-sig strips the BOM correctly regardless of source shell.
    raw = sys.stdin.buffer.readline()
    pat = raw.decode("utf-8-sig").strip()
    if not pat:
...
def fetch_work_item_comments(api_base: str, pat: str, item_id: int, api_version: str) -> list[dict]:
    """Fetch threaded comments (newer ADO Server comments API). Returns [] if unsupported (older TFS)."""
    url = f"{api_base}/wit/workItems/{item_id}/comments?api-version={api_version}"
    try:
        result = http_request(url, pat)
    except RuntimeError:
        return []
    return [
        {"created_by": c.get("createdBy", {}).get("displayName"), "created_date": c.get("createdDate"), "text": c.get("text")}
        for c in result.get("comments", [])
    ]


def fetch_work_item_history(api_base: str, pat: str, item_id: int, api_version: str) -> list[dict]:
    """Fetch revision history and extract the System.History note (if any) from each revision."""
    url = f"{api_base}/wit/workItems/{item_id}/updates?api-version={api_version}"
    result = http_request(url, pat)
    history = []
    for update in result.get("value", []):
        fields = update.get("fields", {})
        note = fields.get("System.History", {}).get("newValue") if "System.History" in fields else None
        if note:
            revised_by = update.get("revisedBy", {}).get("displayName")
            revised_date = update.get("revisedDate")
            history.append({"revised_by": revised_by, "revised_date": revised_date, "note": note})
    return history


def normalize_work_item(raw: dict, comments: list[dict] | None = None, history: list[dict] | None = None) -> dict:
    ...
    item = {
        ...
        "target_release": fields.get("Allscripts.Field.TargetRelease"),
        "injected_in_release": fields.get("Allscripts.Field.InjectedinRelease"),
        "reported_in_release": fields.get("Allscripts.Field.ReportedinRelease"),
        "go_live_blocking": fields.get("Allscripts.Field.GoLiveBlocking"),
        "regulatory": fields.get("Allscripts.Field.Regulatory"),
        "url": raw.get("url"),
    }
...
def main():
    ...
    parser.add_argument("--with-comments", action="store_true", help="Also fetch threaded comments for each work item")
    parser.add_argument("--with-history", action="store_true", help="Also fetch revision history notes (classic System.History) for each work item")
    ...
    try:
        ...
        for item in raw_items:
            item_id = item.get("id")
            comments = fetch_work_item_comments(api_base, pat, item_id, args.api_version) if args.with_comments else None
            history = fetch_work_item_history(api_base, pat, item_id, args.api_version) if args.with_history else None
            work_items.append(normalize_work_item(item, comments=comments, history=history))
