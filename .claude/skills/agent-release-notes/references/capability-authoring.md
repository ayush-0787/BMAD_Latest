...
### Prompt (default)
A markdown file with guidance on what to achieve. Best for judgment-based tasks where you need flexibility — e.g. summarizing a release across multiple work item batches, or writing an executive digest.
...
```
capabilities/
├── changelog-diff.md          # When to run, what to do with results
└── changelog-diff.py          # The actual computation
```
...
capabilities/
└── quarterly-rollup/
    ├── quarterly-rollup.md    # Main guidance
...
- Never write secrets (like the TFS/ADO PAT) to any file a script produces
